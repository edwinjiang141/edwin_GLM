...
with st.container():
    ...
    with col2:
        ...
        st.selectbox("请选择画风：",
                                image_modes,
                                index=0,
                                on_change=lambda:st.session_state["meta"].update(image_mode=st.session_state["image_mode"]),
                                key="image_mode",
                                )
        # End
...
def draw_new_image():
    ...
    # TODO: 加上风格选项
    #image_prompt = '国画' + image_prompt.strip()
    image_prompt = st.session_state["meta"]["image_mode"]+image_prompt.strip()
